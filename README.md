...
7. Избегать сравнение со строчками напрямую
   ...
   **Bad**
    ...
    **Good:**
    ...
    ```   
8. Использовать одинаковый словарь  по всему проекту 
   ...
   **Bad**
    ...
    **Good:**
    ...
    getUser();
    ``` 
9. Иcпользуйте явные наименования 
  **Bad**
    ```
...
### Чистота функций 
1. Используйте внешние зависимости и связи только если это необходимо. Старайтесь сделать функции чистыми. Это позволит избежать лишних мутаций
**Bad**
```
...
merge({ foo: "foo" }, { bar: "bar" }); // => { foo: "foo", bar: "bar" }
```
2. Функция должна представлять только один уровень абстракции.
...
2. Скрывать все, что можно в приватных методах 
...
if (x == undefined) { ... }
```
Не пишите глобальных функций, чтобы избежать конфликты с другими библиотеками
